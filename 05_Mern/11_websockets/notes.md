...
## Websockets:
WebSockets are better because they provide a persistent, full-duplex, low-latency connection that allows real-time two-way communication without repeated requests, unlike traditional HTTP which is stateless and requires polling.
...
CLIENT ←←←←←←← RECEIVE MESSAGE ←←←←←←←←←←←←←←←← SERVER

     (No re-request needed, no polling)
